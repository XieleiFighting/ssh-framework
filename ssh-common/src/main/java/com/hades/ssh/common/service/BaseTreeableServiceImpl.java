...
public abstract class BaseTreeableServiceImpl<T extends BaseEntity<ID> & Treeable<ID>, ID extends Serializable> extends BaseServiceImpl<T, ID> implements BaseTreeableService<T, ID> {
     ...
     * @param newParentIds
     */
    private void updateSelftAndChild(T source, ID newParentId, String newParentIds, int newWeight) {
    ...
    public List<T> findChildren(List<T> parents, Searchable searchable) {
        ...
        List<T> children = findAllWithSort(searchable);
        return children;
    }
